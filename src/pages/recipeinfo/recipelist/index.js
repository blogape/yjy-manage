...
for (let i = 0; i < showAddress.length; i++) {
  children.push(
    <Option key={i.toString(showAddress.length) + i}>{showAddress[i]}</Option>
  );
}
const data = [];
  ...
  render() {
    const columns = [
      ...
      },
      {
        ...
        dataIndex: "action",
        render: (text, record) => (
          <a href="javascript:;" onClick={this.viewClick.bind(text, record)}>
            查看
          </a>
        )
      },
      ...
      <div className="recipelist">
        <Card>
          ...
          <div className="recipelist-header">
            ...
            <Button type="primary" onClick={this.addrepice}>
              添加食谱
            </Button>
